...
import { socialLinks, WhoIs } from "../../../../constants";
        ...
        <div className="md:w-3/5 w-full">
          <div className="md:w-[80%] w-full">
            <p className="text-[16px] font-semibold mb-1">
              Refreegerian, that’s a mouthful isn’t it ?😆 Let me tell you all
              about it!
            </p>
            <p className="text-[15px] mb-2">
              A RefreeGerian is a compassionate and proactive member of the
              RefreeG community dedicated to making a positive impact in Africa.
            ...
            <div className="list space-y-3 mb-6">
              {WhoIs.map((item) => (
                <div className="flex items-center space-x-4" key={item.key}>
                  <Dot size={"size-[15px]"} />
                  <p className="text-[15px] mb-2">{item.list}</p>
                </div>
              ))}
            </div>
            <Link href={socialLinks.community}>
              <Button variant={"text"}>Become a Refreegerian today!</Button>
            </Link>
          </div>
        </div>
