...
const Icon = ({href,children})=>{
  return(
    <Link href={href} className='bg-secondary-9 rounded-full flex items-center justify-center size-[30px] text-white '>{children}</Link>
  )
}
const Footer = () => {
  return (
    <section className='w-full h-full px-[10px] md:px-[50px] py-[25px] mt-[30px] bg-footer'>
      ...
      <div className='md:flex md:justify-between w-full'>
        ...
        <div className='mx-auto md:mx-0 flex space-x-3 items-center w-fit md:justify-normal'>
          ...
          <Icon href={socialLinks.linkedin}><Linkedin width={15} height={15} /></Icon>
          <Icon href={socialLinks.Youtube}><Youtube width={15} height={15} /></Icon>
        </div>
      </div>
    </section>
  )
